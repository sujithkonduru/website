...
function WebServices() {
  ...
  const serviceCategories = {
    ...
    "security": {
      ...
      services: servicesData.security.map((service, index) => ({
        icon: index === 0 ? <Shield className="w-6 h-6 sm:w-8 sm:h-8" /> :
          index === 1 ? <Lock className="w-6 h-6 sm:w-8 sm:h-8" /> :
            index === 2 ? <Database className="w-6 h-6 sm:w-8 sm:h-8" /> :
    ...
    //   services: servicesData.marketing.map((service, index) => ({
    //     icon: index === 0 ? <TrendingUp className="w-6 h-6 sm:w-8 sm:h-8" /> :
    //       index === 1 ? <Users className="w-6 h-6 sm:w-8 sm:h-8" /> :
  ...
  const ServiceCard = ({ service, index, categoryColor }) => (
    <motion.div
      ...
      className="group relative bg-gradient-to-br from-gray-800/50 to-gray-900/50 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-700 hover:border-transparent transition-all duration-300 hover:shadow-2xl hover:shadow-gray-900/50 cursor-pointer backdrop-blur-sm"
    >
      <div className={`absolute inset-0 bg-gradient-to-br ${categoryColor} opacity-0 group-hover:opacity-10 rounded-xl sm:rounded-2xl transition-opacity duration-300`} />
      <div className="relative">
        <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
          <div className={`p-2 sm:p-2.5 rounded-lg sm:rounded-xl bg-gradient-to-br ${categoryColor} bg-opacity-20 group-hover:bg-opacity-30 transition-all`}>
            <div className="text-white scale-90 sm:scale-100">{service.icon}</div>
          </div>
          <h3 className="text-base sm:text-lg font-bold text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-gray-300 transition-all line-clamp-2">
            {service.title}
          </h3>
        </div>
        <p className="text-xs sm:text-sm text-gray-400 mb-3 sm:mb-4 leading-relaxed line-clamp-2">{service.desc}</p>
        <ul className="space-y-1.5 sm:space-y-2">
          {service.features.slice(0, 3).map((feature, j) => (
            <li key={j} className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-gray-400 group-hover:text-gray-300 transition-colors">
        ...
        <div className="mt-3 sm:mt-4 pt-2 sm:pt-3 border-t border-gray-800">
          <button
            ...
            className="text-xs sm:text-sm text-gray-400 hover:text-white flex items-center gap-1 group/btn"
          >
            Learn More
  ...
  const CategoryButton = ({ categoryKey, category }) => (
    <motion.button
      ...
      className={`relative px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-300 flex items-center gap-1 sm:gap-2 whitespace-nowrap ${activeService === categoryKey
        ? `text-white bg-gradient-to-r ${category.color} shadow-lg`
        : "text-gray-400 bg-gray-800/50 hover:bg-gray-700/50 hover:text-white border border-gray-700"
        }`}
    >
      {category.icon}
  ...
  return (
    <div className="bg-gray-950 text-white min-h-screen overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center pt-20 sm:pt-24 lg:pt-32 pb-16 sm:pb-20 px-4 sm:px-6 overflow-hidden">
        {/* Animated Background */}
        ...
        <div className="max-w-6xl mx-auto text-center relative z-10">
          ...
          >
            ...
            <motion.div
              ...
              className="inline-flex items-center gap-1.5 sm:gap-2 bg-white/5 backdrop-blur-sm border border-white/10 rounded-full px-3 sm:px-4 py-1.5 sm:py-2 mb-4 sm:mb-6"
            >
              <Sparkles className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-400" />
              <span className="text-xs sm:text-sm text-yellow-400 font-semibold">Enterprise IT Solutions</span>
            </motion.div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 max-w-3xl mx-auto mb-6 sm:mb-8 px-2 mt-3">
              {[
                ...
                { icon: Zap, value: "120+", label: "Solutions Delivered", color: "purple" },
                { icon: Award, value: "98%", label: "Satisfaction", color: "yellow" }
              ].map((stat, i) => (
                <motion.div
                  key={i}
                  initial={{ opacity: 0, y: 20 }}
                  ...
                  className="bg-white/5 backdrop-blur-sm rounded-lg sm:rounded-xl p-2 sm:p-3 border border-white/10 hover:border-yellow-400/30 transition-all"
                >
                  <stat.icon className={`w-4 h-4 sm:w-5 sm:h-5 text-${stat.color}-400 mx-auto mb-1`} />
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 justify-center items-center px-2">
              <motion.button
                ...
                className="w-full sm:w-auto px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-yellow-400 to-orange-500 text-black rounded-full text-xs sm:text-sm font-semibold hover:shadow-lg hover:shadow-yellow-500/25 transition-all"
              >
                Start Your Project
              </motion.button>
              <motion.button
                ...
                className="w-full sm:w-auto px-4 sm:px-6 py-2.5 sm:py-3 border-2 border-blue-400 text-blue-400 rounded-full text-xs sm:text-sm font-semibold hover:bg-blue-400 hover:text-white transition-all"
              >
      ...
      <section className="sticky top-0 z-40 bg-gray-900/80 backdrop-blur-xl border-y border-gray-800 py-2 sm:py-3 px-2 sm:px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-1 sm:gap-2 overflow-x-auto no-scrollbar pb-1">
            <motion.button
              ...
              className={`flex-shrink-0 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-all ${activeService === "all"
                ...
                {Object.entries(serviceCategories).map(([key, category]) => (
                  <div key={key}>
                    <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6 px-2">
                      <div className={`p-1.5 sm:p-2 rounded-lg bg-gradient-to-br ${category.color} bg-opacity-20`}>
                        {category.icon}
                      </div>
                      <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white">{category.title}</h2>
                      <div className="flex-1 h-px bg-gradient-to-r from-gray-700 to-transparent ml-2 sm:ml-4" />
                    </div>
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 sm:mb-8 px-2">
                  <div className="flex items-center gap-2 sm:gap-3">
                    <div className={`p-2 sm:p-2.5 rounded-xl bg-gradient-to-br ${serviceCategories[activeService].color} bg-opacity-20`}>
                      {serviceCategories[activeService].icon}
                    </div>
                    <div>
                      <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white">{serviceCategories[activeService].title}</h2>
      ...
      {/* Our Approach */}
      <section className="py-12 sm:py-16 px-3 sm:px-4 bg-gray-900">
        <div className="max-w-6xl mx-auto">
          ...
          >
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-3 text-yellow-400">Our Development Approach</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {[
              ...
              <motion.div
                ...
                whileHover={{ y: -5 }}
                className="relative bg-gradient-to-br from-gray-800/50 to-gray-900/50 p-4 sm:p-5 rounded-xl border border-gray-700 hover:border-transparent group transition-all"
              >
                <div className={`absolute inset-0 bg-gradient-to-br ${phase.color} opacity-0 group-hover:opacity-10 rounded-xl transition-opacity`} />
                <div className="relative">
                  <div className="text-2xl sm:text-3xl mb-2 sm:mb-3">{phase.icon}</div>
                  <div className={`text-xs font-bold mb-1 bg-gradient-to-r ${phase.color} bg-clip-text text-transparent`}>
                    {phase.step}
                  </div>
                  <h3 className="text-sm sm:text-base font-bold text-white mb-2">{phase.title}</h3>
                  <p className="text-xs sm:text-sm text-gray-400 leading-relaxed">{phase.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-12 sm:py-16 px-3 sm:px-4">
        <div className="max-w-6xl mx-auto">
          <motion.div
            ...
            className="text-center mb-8 sm:mb-12"
          >
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-3 text-yellow-400">Why Stackenzo?</h2>
            <p className="text-xs sm:text-sm md:text-base text-gray-400 max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {benefits.map((benefit, i) => (
              <motion.div
                ...
                transition={{ delay: i * 0.1 }}
                whileHover={{ y: -5 }}
                className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 p-4 sm:p-5 rounded-xl border border-gray-700 hover:border-yellow-400/50 transition-all"
              >
                ...
                <p className="text-xs sm:text-sm text-gray-400">{benefit.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
  ...
  <div className="max-w-6xl mx-auto">
    <motion.div
      ...
      className="text-center mb-8 sm:mb-12"
    >
      <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-3 text-yellow-400">Client Success Stories</h2>
      <p className="text-xs sm:text-sm md:text-base text-gray-400 max-w-2xl mx-auto">
        ...
        >
          <div className="flex gap-1 mb-2 sm:mb-3">
            {[...Array(testimonial.rating)].map((_, j) => (
              <Star key={j} className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-400 fill-current" />
            ))}
          </div>
          <p className="text-xs sm:text-sm text-gray-300 mb-3 sm:mb-4 italic leading-relaxed">"{testimonial.text}"</p>
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-r from-yellow-400 to-orange-400 flex items-center justify-center text-white font-bold text-sm sm:text-base">
              {testimonial.initials}
            </div>
            <div>
              <div className="text-xs sm:text-sm font-bold text-white">{testimonial.name}</div>
              <div className="text-[10px] sm:text-xs text-gray-400">{testimonial.role}</div>
            </div>
          </div>
        </motion.div>
      ))}
      ...
      {/* <section className="py-12 sm:py-16 px-3 sm:px-4">
        <div className="max-w-6xl mx-auto">
          <motion.div
            ...
            className="text-center mb-8 sm:mb-12"
          >
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-3 text-yellow-400">Our Technology Stack</h2>
            <p className="text-xs sm:text-sm md:text-base text-gray-400 max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
            {[
              ...
              { name: "TensorFlow", icon: "🧠", color: "from-purple-500 to-pink-500" },
              ...
              <motion.div
                ...
                whileHover={{ scale: 1.05 }}
                className={`bg-gradient-to-br from-gray-800/50 to-gray-900/50 p-3 sm:p-4 rounded-lg border border-gray-700 hover:border-transparent group relative overflow-hidden`}
              >
      ...
      <section className="py-12 sm:py-16 px-3 sm:px-4 bg-gray-900">
        <div className="max-w-4xl mx-auto">
          <motion.div
            ...
            className="text-center mb-8 sm:mb-12"
          >
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-3 text-yellow-400">Frequently Asked Questions</h2>
            <p className="text-xs sm:text-sm md:text-base text-gray-400">Got questions? We've got answers.</p>
          </motion.div>
          ...
          <div className="space-y-2 sm:space-y-3">
            {faqs.map((faq, i) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: i * 0.1 }}
                className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 rounded-lg border border-gray-700 overflow-hidden"
              >
                <button
                  onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  className="w-full p-3 sm:p-4 text-left flex justify-between items-center hover:bg-gray-800/50 transition-all gap-2"
                >
            ...
            <div className="relative z-10 p-6 sm:p-8 md:p-10 text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 justify-center items-center">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  onClick={() => setIsQuoteModalOpen(true)}
                  className="w-full sm:w-auto px-4 sm:px-6 py-2 sm:py-2.5 border-2 border-white text-white rounded-full text-xs sm:text-sm font-semibold hover:bg-white hover:text-gray-900 transition-all"
                >
                  Get Free Consultation
                </motion.button>
              </div>
        ...
        {selectedServiceDetail && (
          ...
          >
            <motion.div
              ...
              className="bg-gradient-to-br from-gray-900 to-gray-950 rounded-xl sm:rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto border border-yellow-400/30 shadow-2xl"
            >
              {/* Modal Header */}
              <div className="sticky top-0 bg-gradient-to-r from-yellow-400 to-orange-500 p-3 sm:p-4 flex items-center justify-between z-10">
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className="p-1.5 sm:p-2 bg-white/20 rounded-lg">
                    {selectedServiceDetail.icon}
                  </div>
                  <h2 className="text-sm sm:text-base md:text-lg font-bold text-black line-clamp-1">{selectedServiceDetail.title}</h2>
                </div>
                <button
                  onClick={() => setSelectedServiceDetail(null)}
              ...
              <div className="p-3 sm:p-4 space-y-3 sm:space-y-4">
                ...
                <div className="bg-gray-800/50 p-3 sm:p-4 rounded-lg border border-gray-700">
                  <h3 className="text-xs sm:text-sm font-semibold text-yellow-400 mb-1 sm:mb-2">Service Overview</h3>
                ...
                {/* Key Features */}
                <div>
                  <h3 className="text-xs sm:text-sm font-semibold text-yellow-400 mb-1 sm:mb-2">Key Features</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2">
                    {selectedServiceDetail.features.map((feature, i) => (
                      <div key={i} className="flex items-start gap-1.5 sm:gap-2 bg-gray-800/30 p-2 rounded-lg border border-gray-700">
                ...
                {selectedServiceDetail.benefits && (
                  <div>
                    <h3 className="text-xs sm:text-sm font-semibold text-yellow-400 mb-1 sm:mb-2">Benefits</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2">
                      {selectedServiceDetail.benefits.map((benefit, i) => (
                        <div key={i} className="flex items-start gap-1.5 sm:gap-2 text-gray-300">
                ...
                {selectedServiceDetail.technologies && (
                  <div>
                    <h3 className="text-xs sm:text-sm font-semibold text-yellow-400 mb-1 sm:mb-2">Technologies</h3>
                    <div className="flex flex-wrap gap-1 sm:gap-1.5">
                      {selectedServiceDetail.technologies.map((tech, i) => (
                        <span key={i} className="px-2 sm:px-2.5 py-0.5 sm:py-1 bg-gray-800 text-gray-300 text-xs rounded-full border border-gray-700">
                          {tech}
                        </span>
                      ))}
                    </div>
                ...
                {selectedServiceDetail.timeline && (
                  <div className="bg-gray-800/50 p-2 sm:p-3 rounded-lg border border-gray-700">
                    <div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm text-gray-300">
                      ...
                      <span className="font-semibold text-yellow-400">Timeline:</span>
                ...
                <div className="pt-2 sm:pt-3 border-t border-gray-800">
                  <button
                    ...
                    className="w-full px-3 sm:px-4 py-2 sm:py-2.5 bg-gradient-to-r from-yellow-400 to-orange-500 text-black rounded-lg text-xs sm:text-sm font-semibold hover:shadow-lg transition-all flex items-center justify-center gap-1 sm:gap-2"
                  >
        ...
        {isQuoteModalOpen && (
          ...
          >
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              onClick={(e) => e.stopPropagation()}
              className="bg-gradient-to-br from-gray-900 to-gray-950 rounded-xl sm:rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto border border-yellow-400/30 shadow-2xl"
            >
              {/* Modal Header */}
              <div className="sticky top-0 bg-gradient-to-r from-yellow-400 to-orange-500 p-3 sm:p-4 flex justify-between items-center">
                <h2 className="text-sm sm:text-base font-bold text-black">Get a Quote</h2>
                <button
                  onClick={() => setIsQuoteModalOpen(false)}
                  className="text-black hover:text-gray-700 p-1 hover:bg-white/20 rounded-lg transition-all"
                >
              ...
              <form onSubmit={handleQuoteSubmit} className="p-3 sm:p-4 space-y-2 sm:space-y-3">
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-white mb-1">Full Name *</label>
                  <input
                    type="text"
                    required
                    value={quoteFormData.name}
                    onChange={(e) => setQuoteFormData({ ...quoteFormData, name: e.target.value })}
                    className="w-full p-2 sm:p-2.5 rounded-lg bg-gray-800 border border-gray-700 text-white text-xs sm:text-sm focus:border-yellow-400 outline-none transition-all"
                    placeholder="John Doe"
                  />
                </div>

                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-white mb-1">Email *</label>
                  <input
                    type="email"
                    required
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-white mb-1">Phone *</label>
                  <input
                    type="tel"
                    required
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-white mb-1">Company *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-white mb-1">Service *</label>
                  <select
                    required
                    value={quoteFormData.service}
                    onChange={(e) => setQuoteFormData({ ...quoteFormData, service: e.target.value })}
                    className="w-full p-2 sm:p-2.5 rounded-lg bg-gray-800 border border-gray-700 text-white text-xs sm:text-sm focus:border-yellow-400 outline-none"
                  >
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-white mb-1">Message</label>
                  <textarea
                    rows="3"
                    value={quoteFormData.message}
                    onChange={(e) => setQuoteFormData({ ...quoteFormData, message: e.target.value })}
                    className="w-full p-2 sm:p-2.5 rounded-lg bg-gray-800 border border-gray-700 text-white text-xs sm:text-sm focus:border-yellow-400 outline-none resize-none"
                ...
                <div className="flex flex-col sm:flex-row gap-2 pt-2">
                  <button
                    type="button"
                    onClick={() => setIsQuoteModalOpen(false)}
                    className="flex-1 px-3 sm:px-4 py-2 sm:py-2.5 border border-gray-700 text-white rounded-lg text-xs sm:text-sm font-semibold hover:bg-gray-800 transition-all"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="flex-1 px-3 sm:px-4 py-2 sm:py-2.5 bg-gradient-to-r from-yellow-400 to-orange-500 text-black rounded-lg text-xs sm:text-sm font-semibold hover:shadow-lg disabled:opacity-50 transition-all"
                  >
